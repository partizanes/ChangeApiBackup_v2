...
from ssh import runRemoteSshWithShell, runRemoteSshWithShellAnswer
from const import SSH_DIST, LOCAL_DIST, REMOTE_SERVER
from internal import getCurrentDate, getLastDate, getSubDayDate, sizeof_fmt
...
def getBackupExits(username, date):
    return runRemoteSshWithShell('stat {0}/{1}/{2}'.format(SSH_DIST, date, username))
...
def createHardlinkCopy(username):
    ...
    elif(not lastBackupExits(username) and not currentBackupExits(username)):
        mainLog.info("[createHardlinkCopy][{0}] Предыдущая резервная копия не обнаружена. Будет произведен поиск предыдущих дней.".format(username))

        i = 2
        found = False

        while (i < 5 and not found):
            date  = getSubDayDate(i)
            found = getBackupExits(username, date)


            if(found):
                mainLog.info("[createHardlinkCopy][{0}] Найдена резервная копия: {1} ".format(username, date))
                runRemoteSshWithShell('cp -nlar {0}/{1}/{3} {0}/{2}/'.format(SSH_DIST, date, getCurrentDate(), username, REMOTE_SERVER))
...
def createAdditionalCopy():
    ...
    if(datetime.today().strftime("%A") == 'Sunday'):
        ...
        success = runRemoteSshWithShell('nohup cp -lr {0}/{1} {0}/weekly/ > /dev/null 2>&1 &'.format(SSH_DIST, getCurrentDate()))

        mainLog.debug("[weekly] Статус запуска команды копирования: {0}".format(success))
        ...
        if(success):
            ...
            if(answer['success']):
                ...
                more_fifteen_days = current_timestamp - 691200
                ...
                for dateStr in listBackup:
                    ...
                    if(more_fifteen_days > backup_timestamp):
                        ...
                        answer = runRemoteSshWithShellAnswer("nohup ionice -c3 rm -rf {0}/weekly/{1} > /dev/null 2>&1 &".format(SSH_DIST, dateStr))
    ...
    if(datetime.now().day == 1):
        ...
        success = runRemoteSshWithShell('nohup cp -lr {0}/{1} {0}/monthly/ > /dev/null 2>&1 &'.format(SSH_DIST, getCurrentDate()))

        mainLog.debug("[monthly] Статус запуска команды копирования: {0}".format(success))
            ...
            if(answer['success']):
                ...
                for dateStr in listBackup:
                    ...
                    if(more_monthly > backup_timestamp):
                        ...
                        answer = runRemoteSshWithShellAnswer("nohup ionice -c3 rm -rf {0}/monthly/{1} > /dev/null 2>&1 &".format(SSH_DIST, dateStr))
